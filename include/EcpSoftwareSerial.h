...
#define OVERSAMPLE \
  3  // in RX, Timer will generate interruption OVERSAMPLE time during a bit. Thus OVERSAMPLE ticks in a bit. (interrupt
     // not synchronized with edge).
...
enum {
  POLL_SM_IDLE = 0,
  POLL_SM_START_SEQ,
  POLL_SM_WAIT_START_DONE,
  POLL_SM_SEND_ZERO_BYTE,
  POLL_SM_SEND_BYTE_DELAY,
  POLL_SM_START_TX_REQUEST,
...
class EcpSoftwareSerial {
 ...
 public:
  // public methods
  EcpSoftwareSerial(uint16_t receivePin, uint16_t transmitPin, bool rx_inverse_logic = false,
